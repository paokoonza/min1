...
var myBott = ['u716796ea3f754dd74e7a2bcf73e27487'];//TARO MID LU DISINI SUPAYA BISA PKE COMMAND STAFF

const myBot = ['u716796ea3f754dd74e7a2bcf73e27487'];//TARO MID LU DISINI
...
class LINE extends LineAPI {
    ...
    poll(operation) {
        if(operation.type == 25 || operation.type == 26) {
            ...
            if(waitMsg == "yes" && operation.message.from == vx[0] && this.stateStatus.mute != 1){
			    ...
			    this._sendMessage(message,"Me on")
	    ...
	    this.textMessage("0103",seq,operation.param2,1);
          ...
          if(operation.type == 15 && this.stateStatus.bmsg == 1) {
             ...
             out.text = "[บายครับแล้วเจอกันใหม่ครับ]\n\n>> BY. มิน Hack BOT <<\n\n™௮ิю🌹のՃສิ७Ҩ์🌹☆\n\nไม่รู้จะยาวไปใหน" 
            ...
            if(operation.type == 17 && this.stateStatus.bmsg == 1) {
               ...
               kam.text = "»»สวัสดีครับยินดีต้อนรับเข้ากลุ่มครับ\n\n→♡BY. มิน Hack BOT♡←\n\n[™௮ิю🌹のՃສิ७Ҩ์🌹☆]\n\nผมว่ามันก็ยาวเกิ้น55" 
    ...
    async textMessage(textMessages, seq, param, lockt) {
      ...
      if(txt == 'add' && this.sendStaff == 0 && isAdminOrBot(seq.from)){
         this.sendStaff = 1;
       ...
       if(seq.contentType == 13 && this.sendStaff == 1 && isAdminOrBot(seq.from)) {
        ...
        if(txt == 'staff' && this.sendStaff == 0 && isAdminOrBot(seq.from))
{
           this.sendStaff = 2;
           ...
           if(seq.contentType == 13 && this.sendStaff == 2 && isAdminOrBot(seq.from))
{
    ...
    await this._sendMessage(seq,'Sukses Menghapus Staff~');
        ...
        if(txt == 'บอท') {
           if(isAdminOrBot(seq.from) || isStaffOrBot(seq.from)) {
            this._sendMessage(seq, '♡บอทยังอยู่น้าาา♡\n\n●>>BY● Min Hack Bot');
           }
        }
...
');
        ...
        if(txt == 'sp') {
            ...
            await this._sendMessage(seq,'รอสักแปบนะครับ...\n\n● BY ● ™௮ิю🌹のՃສิ७Ҩ์🌹☆');


            const rtime = (Date.now() / 1000000) - curTime;
            await this._sendMessage(seq, `${rtime} ได้แล้วนะครับ`);
        }
...
let { listMember } = await this.searchGroup(seq.to);
     const mentions = await this.mention(listMember);
        ...
        if(txt === 'กูมาบิน' && this.stateStatus.kick == 1 && isStaffOrBot(seq.from)) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        if(txt == 'me') {
           this._sendMessage(seq,'กำเบย');
           seq.contentType=13;
           seq.contentMetadata = { mid: 'u716796ea3f754dd74e7a2bcf73e27487' };
           this._client.sendMessage(1, seq);
        }
